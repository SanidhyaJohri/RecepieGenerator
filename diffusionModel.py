...
def app():
	...
	input = st.text_input("Enter the prompt: ", placeholder="How to make Momos recipe?")
	...
	grid_size = (len(imageslist), 1)
	grid_width = 300 * grid_size[0]
	grid_height = 300 * grid_size[1]
	grid_image = Image.new("RGB", (grid_width, grid_height), color="white")
	with open("newprompt", "rb") as f2:
		prompt = pickle.load(f2)
	j = 0
	for i in range(0, len(imageslist)):
		imagen = Image.open(imageslist[i])
		imagen = imagen.resize((300, 300))
		draw1 = ImageDraw.Draw(imagen)
		text1 = prompt[i]
		draw1.text((40, 40), text1, fill=(255, 255, 255))
		grid_image.paste(imagen, (j, 0))
		j = j+ 300
	grid_image.save("image_grid2.png")